...
(function () {
    ...
    var fsoBuffer = function (data, offset, error) {
        ...
        if (offset + len > 0) {
            ...
            buffered[i++] = new Blob([new ArrayBuffer(offset), new Int8Array(data, 0, size)]);
            len -= size;
            begin = size;
    ...
    var fsoCommands = {
        getBytes: function (callback) {
            ...
            if (this.parent.persistent) {
                navigator.webkitPersistentStorage.queryUsageAndQuota(function (used, available) {
                    fso.availableBytes = available;
                    execute(used, available);
                }, errorHandler);
            }
            else {
                navigator.webkitTemporaryStorage.queryUsageAndQuota(function (used, available) {
                    fso.availableBytes = available;
                    execute(used, available);
                }, errorHandler);
            }
        ...
        info: function (path, callback) {
            ...
            getFile = function () {
                fs.root.getFile(path, {}, function (fileEntry) {
                    fileEntry.getMetadata(getInfo.bind(fileEntry), errorHandler);
                }, errorHandler);
            };
            getDir = function () {
                fs.root.getDirectory(path, {}, function (dirEntry) {
                    dirEntry.getMetadata(getInfo.bind(dirEntry), errorHandler);
                }, function (e) {
        ...
        put: function (file, path, name, callback) {
            ...
            this.fs.root.getFile(path + (name ? name : file.name), { create: true, exclusive: false }, function (fileEntry) {
        ...
        rm: function (filename, callback) {
            ...
            getFile = function () {
                fs.root.getFile(filename, {}, function (fileEntry) {
                    fileEntry.remove(execute.bind(null, true), errorHandler);
                }, function (e) {
            ...
            getDir = function () {
                fs.root.getDirectory(filename, {}, function (dirEntry) {
                    dirEntry.remove(execute, function (e) {
        ...
        mkdir: function (dirname, callback) {
            ...
            var fs = this.fs;
            makeDir = function () {
                fs.root.getDirectory(dirs.join('/'), { create: true }, function () {
                    if (create.length) {
        ...
        rmdir: function (dirname, callback) {
            ...
            if (dirname === '' || dirname === '/') {
                ...
                dirReader.readEntries(function (entries) {
                    ...
                    if (!len) {
                        ...
                        for (var i = 0; i < len; i++) {
                            var entry = entries[i];
                            if (entry.isDirectory) {
                                entry.removeRecursively(increment, errorHandler);
                            }
                            else {
                                entry.remove(increment, errorHandler);
                            }
                        }
                    }
                }, errorHandler);
                ...
                fs.root.getDirectory(dirname, {}, function (dirEntry) {
                    dirEntry.removeRecursively(execute, errorHandler);
                }, function (e) {
                    if (e.name === 'NotFoundError') {
                        execute();
                    }
        ...
        rename: function (path, name, callback) {
            ...
            var getDirectory = function () {
                fs.root.getDirectory(path, {}, function (dirEntry) {
                    if (!movePath) {
                        dirEntry.moveTo(fs.root, name, execute.bind(null, path, name), function (e) {
                            if (e.name === 'InvalidModificationError') {
                        ...
                        fs.root.getDirectory(movePath, {}, function (dirEntryTo) {
                            dirEntry.moveTo(dirEntryTo, name, execute.bind(null, path, name), function (e) {
                                if (e.name === 'InvalidModificationError') {
                                    errorHandlerTo('File ' + name + ' already exists');
                                }
                                else {
            ...
            fs.root.getFile(path, {}, function (fileEntry) {
                if (!movePath) {
                    fileEntry.moveTo(fs.root, name, execute.bind(null, path, name), function (e) {
                        if (e.name === 'InvalidModificationError') {
                            errorHandlerTo('File ' + name + ' already exists');
                        }
                        else {
                    ...
                    fs.root.getDirectory(movePath, {}, function (dirEntry) {
                        fileEntry.moveTo(dirEntry, name, execute.bind(null, path, name), function (e) {
                            if (e.name === 'InvalidModificationError') {
                                errorHandlerTo('File ' + name + ' already exists');
                            }
                            else {
        ...
        move: function (path, movePath, newName, callback) {
            ...
            var getDirectory = function () {
                fs.root.getDirectory(path, {}, function (dirEntry) {
                    fs.root.getDirectory(movePath, {}, function (dirEntryTo) {
                        dirEntry.moveTo(dirEntryTo, newName, execute.bind(null, path, name), function (e) {
                            if (e.name === 'InvalidModificationError') {
            ...
            fs.root.getFile(path, {}, function (fileEntry) {
                fs.root.getDirectory(movePath, {}, function (dirEntry) {
                    fileEntry.moveTo(dirEntry, newName, execute.bind(null, path, name), function (e) {
                        if (e.name === 'InvalidModificationError') {
                            errorHandlerTo('File already exists');
                        }
                        else {
        ...
        copy: function (path, movePath, newName, callback) {
            ...
            var getDirectory = function () {
                fs.root.getDirectory(path, {}, function (dirEntry) {
                    fs.root.getDirectory(movePath, {}, function (dirEntryTo) {
                        dirEntry.copyTo(dirEntryTo, newName, execute.bind(null, path, movePath), function (e) {
                            if (e.name === 'InvalidModificationError') {
                                errorHandlerTo('File already exists');
                            }
                            else {
            ...
            fs.root.getFile(path, {}, function (fileEntry) {
                fs.root.getDirectory(movePath, {}, function (dirEntry) {
                    fileEntry.copyTo(dirEntry, newName, execute.bind(null, path, movePath), function (e) {
                        if (e.name === 'InvalidModificationError') {
                            errorHandlerTo('File already exists');
                        }
                        else {
        ...
        list: function (dirname, maxDepth, callback) {
            ...
            if (dirname === '' || dirname === '/') {
                ...
                fs.root.getMetadata(function (data) {
                ...
                fs.root.getDirectory(dirname, {}, function (dirEntry) {
                    ...
                    baseObject.isFile = false;
                    baseObject.isDirectory = true;
                    dirEntry.getMetadata(function (data) {
                        baseObject.size = data.size;
                        baseObject.modified = data.modificationTime;
    ...
    function FSO(size, persistent, successCallback, errorCallback) {
        ...
        requestFileSystem(this.persistent ? pConst : tConst, size, this.__init__.bind(this, size, persistent), this.__error__.bind(this));
    }
    ...
    FSO.prototype.__init__ = function (size, persistent, fs) {
        ...
        if (persistent) {
            navigator.webkitPersistentStorage.requestQuota(size, this.__requestStorage__.bind(this, size), this.__error__.bind(this));
        }
        else {
            navigator.webkitTemporaryStorage.requestQuota(size, this.__requestStorage__.bind(this, size), this.__error__.bind(this));
        }
    };
